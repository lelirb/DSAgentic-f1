...
export function evaluateD3(input, rules) {
  ...
  const sub = {
    ...
    api_consistency: { points: apiConsistency === null ? null : apiConsistency * w.api_consistency, max: w.api_consistency },
...
function estimateApiConsistency(components) {
  ...
  if (totalPropUses === 0) return null; // sin propiedades no hay nada que comparar
