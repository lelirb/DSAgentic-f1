...
export async function crawl(entryUrl, options = {}, fetchImpl = fetch) {
  ...
  const deadline = opts.max_duration_ms ? Date.now() + opts.max_duration_ms : Infinity;
  ...
  while (queue.length > 0) {
    ...
    const timeoutMs = Number.isFinite(remainingMs) ? Math.max(1000, Math.min(opts.request_timeout, remainingMs)) : opts.request_timeout;
    const results = await Promise.all(
      batch.map((item) => processOne(item, { ...opts, request_timeout: timeoutMs }, visited, pageRecords, pages, fetchImpl, ctx))
    );
    ...
    for (const r of results) {
      ...
      for (const link of links) {
        ...
        if (queued.has(link)) continue; // already waiting in the queue
        const u = new URL(link);
        if (u.search && !QUERY_IS_PAGE.test(u.search.slice(1))) {
          const key = `${u.origin}${u.pathname}`;
          const n = variantsPerPath.get(key) || 0;
          if (n >= opts.max_query_variants_per_path) continue;
          variantsPerPath.set(key, n + 1);
        }
        queued.add(link);
  ...
  const probeOpts = timeLimited ? { ...opts, request_timeout: Math.min(opts.request_timeout, 3000) } : opts;
  await probeAgentManifests(probeOpts, visited, pageRecords, pages, fetchImpl, ctx);
