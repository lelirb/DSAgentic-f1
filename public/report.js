...
(function () {
  ...
  // Qué puede observar HOY el rastreo en vivo (ver engine/src/extractor):
  // - unreadable: el extractor deja estos campos vacíos siempre (props, variants,
  //   states, composition, patterns, accessibility, ejemplos ejecutables).
  // - linked: solo se detecta si la URL del recurso aparece enlazada en lo rastreado.
  // - (resto) observable: se busca en el texto de las páginas que sí se leyeron.
  const LIVE_UNREADABLE = new Set([
    "D1.props_types_accessibility", "D3.*", "D5.*",
    "D6.executable_examples", "D6.variants_states_explained", "D6.accessibility",
  ]);
  const LIVE_LINKED = new Set([
    "D1.component_index", "D1.structured_tokens", "D1.types_or_schema", "D1.agent_interface",
    "D1.agent_manifest", "D2.*", "D6.versioning",
  ]);
  const inSet = (set, d, k) => set.has(`${d}.${k}`) || set.has(`${d}.*`);
  const scopeOf = (d, k) => (inSet(LIVE_UNREADABLE, d, k) ? "unreadable" : inSet(LIVE_LINKED, d, k) ? "linked" : "observable");
  ...
  function classify(dimKey, dim, ctx) {
    ...
    for (const [key, sc] of Object.entries(subs)) {
      const scope = ctx.mode === "live" ? scopeOf(dimKey, key) : "provided";
      let cert, reason = null;
      if (!isNum(sc.points)) { cert = "notEvaluated"; reason = "rsExcluded"; }
      else if (key === "disambiguation" && sc.applicable === false) cert = "na";
      else if (dimKey === "D3" && key === "api_consistency" && dim.coverage && dim.coverage.components_with_props === 0) {
        cert = "notEvaluated"; reason = "noBasisApi"; // 0 porque no hubo propiedades que comparar
      }
      else if (sc.points >= sc.max - 1e-9) cert = "demonstrated";
      else if (sc.points > 0) cert = "partial";
      else if (ctx.mode === "live") {
        if (scope === "unreadable") { cert = "notEvaluated"; reason = "rsUnreadable"; }
        else { cert = "notDemonstrated"; reason = scope === "linked" ? "rsLinked" : "rsObservable"; }
  ...
  function incompleteReasons(report, ctx, analysis) {
    ...
    if (all.some((c) => c.items.some((i) => i.reason === "rsUnreadable"))) out.push(u.rUnreadable);
    if (all.some((c) => c.items.some((i) => i.reason === "rsLinked") || (c.whole === "notDemonstrated" && ctx.mode === "live"))) out.push(u.rLinked);
  ...
  function renderChain(report, analysis) {
    const u = T().ui;
    const items = ALL.map((k) => {
      ...
      const inc = analysis[k] && analysis[k].incomplete ? `<div class="inc">${esc(u.dimIncomplete)}</div>` : "";
      return `<a href="#dim-${k}">
  ...
  function renderTech(k, d, cls, findings) {
    ...
    const fHtml = dimFindings.length
      ? `<p class="tech-meta"><strong>${esc(u.engineFindings)}</strong>${u.engineLangNote ? ` · ${esc(u.engineLangNote)}` : ""}</p>` +
        dimFindings.map((f) => `<div class="finding"><code>${esc(f.severity)}</code>${f.component ? ` · ${esc(f.component)}` : ""} — ${esc(f.finding)}${f.recommendation ? `<br>→ ${esc(f.recommendation)}` : ""}</div>`).join("")
      : "";
  ...
  function renderLimitations(report, ctx, analysis) {
    ...
    const unread = (k) => analysis[k] && analysis[k].items.some((i) => i.reason === "rsUnreadable");
    if (unread("D3")) items.push(u.limD3Live);
    if (unread("D5")) items.push(u.limD5Live);
    items.push(u.limD7);
