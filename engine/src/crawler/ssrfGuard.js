 ...
 * link-local, or otherwise non-public address. Resolves DNS once, up front —
 * this does NOT protect against DNS rebinding (hostname resolves to a public
 * IP at check time, then to a private one at fetch time) or SSRF via HTTP
 * redirect chains that hop to an internal host after the initial check. Both
 * are known residual risks or a fetching layer that re-validates on every
 * hop (documented, not silently claimed as solved).
 */
