import { extractTables, tokensFromTables } from "./readPage.js";
  ...
  for (const page of htmlPages) {
    for (const row of tokensFromTables(extractTables(page.body))) {
      ...
      const id = evidenceCollector.add({
        source: page.url, location: row.name, content: JSON.stringify({ value: row.value, description: row.description }),
